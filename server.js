...
app.use(
  session({
    //Sets up a temporary memory space for this user's session — needed so flash messages (step 6) have somewhere to live.
...
app.use((req, res, next) => {
  ...
  if (token) {
    // if token is there try to verify it or if has been tempered with
    try {
      req.user = jwt.verify(token, process.env.JWT_SECRET || "fallback"); // verify and se if expired or not
    } catch {
      // if token is not valid set req.user to null- treat as logged out
